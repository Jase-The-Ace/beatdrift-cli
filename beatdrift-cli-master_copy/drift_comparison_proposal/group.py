...
def get_state_groups(state):
    with (open(state, 'r')) as file:
        ...
        for resource in dict['resources']:
            if resource['type'] == "aws_iam_group_membership" and resource[
                    'mode'] == "managed":
                for instance in resource['instances']:
                    state_groups.append({
                        ...
                        'users':
                        [x.lower()
                         for x in instance['attributes']['users']].sort(),
                    })
    return state_groups
...
def get_state_group_policy_attachments(state):
    with (open(state, 'r')) as file:
        ...
        group_list = []
        for resource in dict['resources']:
            if resource['type'] == "aws_iam_group" and resource[
                    'mode'] == "managed":
                for instance in resource['instances']:
                    group_list.append(str.lower(
                        instance['attributes']['name']))
        for group in group_list:
            policy_list = []
            for resource in dict['resources']:
                if resource[
                        'type'] == "aws_iam_group_policy_attachment" and resource[
                            'mode'] == "managed":
                    for instance in resource['instances']:
                        if str.lower(instance['attributes']['group']) == group:
                            policy_list.append(
                                str.lower(
                                    instance['attributes']['policy_arn']))
            policy_list.sort()
...
def compare_groups(list1, list2):
    ...
    for group in groups:
        print(Fore.RED + "Group: " + str(group['group']) + ", Users: " +
              str(group['users']))
            ...
            if item1['group'] == item2['group']:
                if item1['policies'] != item2['policies']:
                    print(Back.GREEN + item1['group'] + Style.RESET_ALL +
                          ":\n" + Fore.GREEN + "AWS: " +
                          str(item1['policies']) + "\n" + Fore.RED + "TF:  " +
                          str(item2['policies']) + Style.RESET_ALL)
